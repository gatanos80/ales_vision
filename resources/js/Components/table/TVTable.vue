<template>
    ...
    <div v-if="!hidePagination && totalRows != 0 && !hideSummary && !busy" class="flex items-center justify-end mr-2 mb-2 text-sm">
       <span>
        <div class="m-5">
        <select v-model="localPerPage" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full">
...
watch(() => localCurrentPage.value, (value) => {
    refreshCounter()
    emit('changePage', { perPage: localPerPage.value,  page: value, from: fromRow.value, to: toRow.value})
    emit('update:currentPage', value)
}, {immediate: true})
watch(() => localPerPage.value, (value) => {
    refreshCounter()
    emit('changePage', {perPage: value, page: localCurrentPage.value, from: fromRow.value, to: toRow.value})
    emit('update:currentPage', value)
}, {immediate: true})
